...
namespace System.Data.Common
{
    internal static class ADP
    {
        ...
        static internal void CheckArgumentLength(string value, string parameterName)
        {
            ...
            if (0 == value.Length)
            {
                throw Argument(SR.Format(Resources.Strings.ADP_EmptyString, parameterName));
            }
        }
        ...
        {
            return Argument(SR.Format(Resources.Strings.ADP_ConnectionStringSyntax, index));
        }
        static internal ArgumentException KeywordNotSupported(string keyword)
        {
            return Argument(SR.Format(Resources.Strings.ADP_KeywordNotSupported, keyword));
        }
        static internal ArgumentException InvalidKeyname(string parameterName)
        {
            return Argument(SR.Format(Resources.Strings.ADP_InvalidKey), parameterName);
        }
        static internal ArgumentException InvalidValue(string parameterName)
        {
            return Argument(SR.Format(Resources.Strings.ADP_InvalidValue), parameterName);
        }
        static internal ArgumentException ConvertFailed(Type fromType, Type toType, Exception innerException)
        {
            return ADP.Argument(SR.Format(Resources.Strings.SqlConvert_ConvertFailed, fromType.FullName, toType.FullName), innerException);
        }
...
//          UnexpectedTransactedObject = 11,
        ...
        {
            return InvalidOperation(SR.Format(Resources.Strings.ADP_InternalProviderError, (int)internalError));
        }
        ...
        {
            return ArgumentOutOfRange(SR.Format(Resources.Strings.ADP_InvalidSourceBufferIndex, maxLen.ToString(CultureInfo.InvariantCulture), srcOffset.ToString(CultureInfo.InvariantCulture)), parameterName);
        }
        ...
        {
            return IndexOutOfRange(SR.Format(Resources.Strings.SQL_InvalidDataLength, length.ToString(CultureInfo.InvariantCulture)));
        }
        static internal IndexOutOfRangeException InvalidBufferSizeOrIndex(int numBytes, int bufferIndex)
        {
            return IndexOutOfRange(SR.Format(Resources.Strings.SQL_InvalidBufferSizeOrIndex, numBytes.ToString(CultureInfo.InvariantCulture), bufferIndex.ToString(CultureInfo.InvariantCulture)));
        }
        static internal ArgumentOutOfRangeException InvalidDestinationBufferIndex(int maxLen, int dstOffset, string parameterName)
        {
            return ArgumentOutOfRange(SR.Format(Resources.Strings.ADP_InvalidDestinationBufferIndex, maxLen.ToString(CultureInfo.InvariantCulture), dstOffset.ToString(CultureInfo.InvariantCulture)), parameterName);
        }
